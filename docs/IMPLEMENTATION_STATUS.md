...
✅ **14/14 automated RLS isolation tests pass** against real Postgres (`pnpm test:rls`, `RLS_TEST_BACKEND=local` in this sandbox — see `docs/DECISIONS.md`): cross-user project/chapter/scene/story-bible access denied, AI conversations/messages/document_chunks isolated per project+user, the ranked full-text search functions respect RLS, a client cannot self-grant a paid entitlement, series-ownership IDOR blocked by a CHECK constraint, soft-delete recovery bin is private. **Verified: Auto.**
