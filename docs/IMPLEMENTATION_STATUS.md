...
✅ Theme/reduced-motion cross-device sync: the `preferences` table (theme, reduced_motion) existed since early in the project, RLS-protected, but every client only ever read/wrote `localStorage` — a cloud author's dark-mode choice never followed them to a second device. Found via the same unused-table audit as `writing_sessions`/`parts`. `loadRemotePreferences`/`saveRemotePreferences` (`apps/web/src/lib/repos/preferences.ts`) now sync it for cloud accounts; `localStorage` stays the source of truth for instant paint on load (no flash of default theme while the async fetch resolves), and the remote fetch reconciles shortly after mount if it differs. Local-only mode skips this entirely (no account to sync to). **Verified: Auto** (`preferences.test.ts` against a mocked Supabase client); not exercised against a live Supabase project with two real devices — **Prod creds required** for that half.
...
❌ Custom user-defined field _definitions_ (`custom_field_defs` table exists, RLS-protected) have no UI yet — today "custom fields" means freeform notes/tags only, not admin-defined structured fields per project.
...
✅ AI Findings workspace: real rule-based scanner (duplicate names, orphaned open threads, timeline conflicts) runs with zero AI cost; accept/dismiss/snooze/convert-to-task/mark-intentional all persist. Re-running the scan is idempotent and respects the author's prior triage: each finding is keyed by its finding type plus the sorted set of the real-world entities it's evidence for, so a re-scan neither duplicates a still-open finding nor resurrects one already dismissed/accepted/snoozed/marked-intentional (a real bug until 2026-09-24 — see `docs/DECISIONS.md` — unlike the deliberately-different, documented behavior of the appearances scanner above, which does intentionally resurface dismissed suggestions). The model-backed consistency-check family of AI Assistant modes now also feeds this same table: the system prompt asks for a trailing structured findings block (only for modes where that's meaningful — `consistency_check`, `character_continuity`, `timeline_analysis`, `plot_thread_tracking`, `dropped_thread_detection`, `canon_extraction`), which `extractFindings` strips out of the visible answer and persists as real `ai_findings` rows with resolved citations — no background job needed, since it's post-processing of a response the author's own request already produced. **Verified: Auto** (`findingsScanner.test.ts` — detection + dedup-against-repeat-scans + dedup-against-dismissed; `findingsExtraction.test.ts`, `aiClient.test.ts` covering the full local pipeline; RLS proven). See `docs/AI_ARCHITECTURE.md` and `docs/DECISIONS.md`.
...
✅ Storyboard: cards grouped into columns, structure overlays (Three-Act / Save the Cat / Hero's Journey / none) as **non-restrictive** relabeling, pointer drag-and-drop reorder (dnd-kit) _and_ a fully keyboard-operable up/down-arrow alternative plus a column `<select>` for moving between columns without dragging.
...
fallback the way web has). Every core module from the product spec now has _some_ mobile screen, each
...
professional-review-pending legal drafts all exist. What's honestly _not_ yet true: no live Supabase project
